...
def generate_excel(df_output, alertas=None, df_merged=None):
    ...
    if has_data:
        d = df_output.copy()
        for col in ('cajas_fase1', 'cajas_fase2', 'cajas_fase3', 'cajas_fase4', 'vida_util'):
    ...
    if has_data:
        ...
        resumitem_df = pd.DataFrame(columns=[
            ...
            'Cajas Fase 1', 'Cajas Fase 2', 'Cajas Fase 3', 'Cajas Fase 4',
        ])
    ...
    _COMPRADOR_COLS = [
        ...
        'Exceso Cajas Est. Total', '# Tiendas Bloqueadas por MAX',
        'Nivel de Riesgo', 'Recomendación Compra',
    ]
    if has_data and df_merged is not None:
        ...
        # Tiendas que el tope MAX dejó fuera del reparto normal de este ítem
        # (ya tenían en existencias tanto o más que su máximo). Informativo
        # para el comprador: explica por qué un ítem se repartió entre menos
        # tiendas de las que tiene activas en el portafolio.
        mn['_bloq_max'] = (
            mn['bloqueado_por_max'].fillna(False).astype(int)
            if 'bloqueado_por_max' in mn.columns else 0
        )
        ...
        comprador_out = pd.DataFrame({
            ...
            '# Tiendas Bloqueadas por MAX': cb['n_bloq_max'],
    ...
    with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
        ...
        resumitem_df.to_excel(writer, index=False, sheet_name='Resumen Ítems', startrow=5)
        ...
        _notas = [
            ...
            f'Fase 3 — Excedente voluntario: el sobrante se distribuye por rondas proporcional a '
            ...
            'cerrando la brecha en vez de mantenerla. Nunca deja a una tienda por encima del tope.',
            'Fase 4 — Cero residuo forzado: si tras la Fase 3 todavía sobran cajas (ninguna '
            'tienda podía recibir más sin superar el tope), se reparte lo que falte por menor '
            'días proyectados, incluso por encima del tope — es la única fase que lo permite. '
            'Un ítem con mucha Fase 4 indica sobre-compra frente al portafolio real.',
        ]
        ...
        # Encabezado de "Resumen Ítems" desplazado por las notas (fila 6 en Excel).
        for cell in ws_ri[6]:
            cell.fill = HEADER_FILL
            cell.font = HEADER_FONT
            cell.alignment = HEADER_ALIGN
